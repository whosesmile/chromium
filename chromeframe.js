...
(function ($) {
  ...
  $.fn.activebar = function (options) {
    ...
    $.fn.activebar.iframe = $('<iframe src="' + (options.url) + '" frameborder="0" border="0" style="border:none"></iframe>');
  ...
  $('head script').each(function () {
    ...
    if (path.indexOf('chromeframe.js') !== -1) {
      $.fn.activebar.defaults.url = path.substring(0, path.lastIndexOf('/')) + '/chromium.msi';
      $.fn.activebar.defaults.icons_path = path.substring(0, path.lastIndexOf('/')) + '/';
    }
  });
...
(function ($) {
  ...
  $.chromeframebar.defaults = {
    'language': true,
    'text': {
      'zh-CN': '为了更好的支持低版本浏览器，您需要安装插件： 点击下载',
